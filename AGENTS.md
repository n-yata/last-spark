...
AGENTS.md、`.agents/` 配下のスキル、docs 配下の日本語文書などが文字化けして見える場合、まずツール側のエンコーディング指定を疑うこと。
PowerShell で読む場合は `Get-Content -Encoding UTF8` を使い、必要に応じて別の読み取り方法でも確認する。
ユーザー側で文字化けしていない可能性があるため、確認前に「ファイルが壊れている」「ユーザー環境で文字化けしている」と断定しないこと。
Claude Code など他ツールでは正常に読める場合がある。Codex 側の表示・読み取り手順の問題として扱い、UTF-8 で再読込してから判断すること。
...
`.Codex/` 配下の command / skill / agent は `.Codex/README.md` に目次化している。
**command / skill / agent を追加・削除・リネームしたら、`.Codex/README.md` も必ず同じ変更で更新する**こと（更新漏れでカタログが陳腐化するのを防ぐ）。
...
**ポイント**: スペック駆動開発の詳細を意識する必要はありません。Codexが適切なスキルを判断してロードします。
