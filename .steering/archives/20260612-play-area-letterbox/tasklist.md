...
## 追加修正2(実機フィードバック対応 2026-06-12)

シャビ(スマホ実機)から「全体的に表示が縮小されすぎて見づらい」「帯をもう少し狭めて」。

- [x] 原因: ズームを `viewH/GAME_HEIGHT`(帯で狭めた高さに全高540を詰める)にしていたため、帯ぶん世界が縮小。
- [x] ズーム基準を `scale.height/GAME_HEIGHT`(画面全体基準)へ変更。帯は viewport を下から削るだけにし、世界の見かけ倍率を帯なしと同一に保つ。displayWidth も帯なし相当に戻りボストリガーのズレも解消。
- [x] 帯を狭める: `CONTROL_BAND_RATIO` 0.18→0.14、`MIN` 104→96、`MAX` 140→112。
- [x] テスト期待値更新(98/MAX/MIN)。Playwright目視: モバイルで世界が帯なしと同サイズに拡大・帯が狭く・地面/プレイヤー/足場とも視認可。
- [x] `functional-design.md`: ズーム基準の方針を明記。
