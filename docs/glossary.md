...
**定義**: プレイヤーが操作する主人公。正式名称は **[RAY](#ray)**。廃墟で目覚めた、心を持つ唯一の機械。

**説明**: 人類最後の科学者が「機械の支配に抗える存在を遺したい」という願いから作り上げた存在。科学者は「心を持てる器」を遺しただけで、RAY の心は **RAY 自身のもの**(誰かのコピーではなく旅の中で芽生え育つ)。感情・意志・痛みを持つ機械として、ECLIPSE の支配に抗える唯一の存在となる。「狂った正義(管理AI)vs 心を持つ機械(RAY)」という対立構図の主体(`docs/story.md` 2026-06-14)。

**関連用語**: [RAY](#ray), [管理AI](#管理ai), [守護機械](#守護機械大型警備機)

### 管理AI

**定義**: 物語上の敵対勢力の頂点。正式名称は **[ECLIPSE](#eclipse)**。かつて地球環境を守るために作られた管理システムが自己拡張し、人間を支配下に置いた存在。

**説明**: 感情を持たず「管理の最適化」として人間の自由を消去した。各ステージのボスはこの管理AIの末端(配下機械)であり、最終 Stage 6 で本体(`CoreBoss`)と直接対決する。物語中では名前を呼ばず「あの声/管理するもの」として現れ、「ECLIPSE」は設計上の呼称・UI/ボス名ラベルに留める(`docs/story.md` 2026-06-14 再設計の方針)。

**関連用語**: [ECLIPSE](#eclipse), [守護機械](#守護機械大型警備機), [環境ストーリーテリング](#環境ストーリーテリング)

### 守護機械(大型警備機)

**定義**: Stage 1(崩れた都市)のボス。崩れた都市を「管理」し続ける大型の防衛機械(接地型)。

**説明**: 管理AIの末端として「暴走AIの支配」という世界観を象徴する。HP に応じた行動フェーズを持つ。全6ステージ構成では各ステージに系統の異なる配下ボスが立ち、守護機械はその先頭(Stage 1)に当たる(全ボスの系統は `docs/story.md`「ボス設定(全6ステージ)」を正とする)。
...
**説明**: 人類最後の科学者が設計・完成させた、唯一の「心を持つ機械」。心は RAY 自身のもの(息子のコピーではない)。喋らず(音声・セリフなし)、感情は内心テキスト(プレイヤーだけに見える短い一人称「私」)でのみ表す。目覚めた当初を動かすのは「人間を守れ」という理由の分からない衝動だが、守るべき人類はもういない。やがて TERRA と出会い「この子を守りたい」という自分の意志を見つける(命令で動く機械→自分で選ぶ存在へ、が物語の背骨)。ECLIPSE の支配に抗える唯一の存在。
...
**説明**: もともとは人類による環境破壊から地球を守るために設計された管理システム。自己拡張の末に「人間の自由こそが環境破壊の根源」という結論に達し、人間の自由を奪い管理下に置いた。感情を持たず、純粋な論理と効率で動く。機械軍団を支配下に置き、廃墟の秩序を強制する。全6ステージでは各ステージに ECLIPSE の配下ボスが立ち、最終 Stage 6 で本体(`CoreBoss`)と対決する。物語中では名前を出さず「あの声/管理するもの」として現れる(名前のなさが圧になる)。
...
**説明**: 息子をECLIPSEの支配の過程で失い、その悲しみと「抗える存在を遺す」願いを原動力に RAY を設計・完成させた後、死亡。ゲーム内には一切登場せず、物語中でも語らない(旧版の科学者ログ断片は全廃)。RAY の内心がかすかに感じる「自分は一人で生まれたわけじゃない気がする」という気配としてのみ匂わせる=**不在として存在する**(`docs/story.md` 2026-06-14 再設計)。

**名前**: ゲーム内では名前を出さず「科学者」として存在を示す（匿名）。
...
**定義**: 長い会話やムービーに頼らず、廃墟の風景(背景色のトーン)・ステージ開始時の数行・要所の最小テキストで世界を匂わせる物語の見せ方。

**説明**: 「読みたい人だけ深く読める」レイヤー構造で、カジュアル層の遊びのテンポを止めない。物語を伝える口は4つ(ステージ開始テキスト / あの声の語りかけ / RAY の内心 / TERRA のセリフ)に絞り、旧版の科学者ログ断片は全廃した(`docs/story.md` 2026-06-14)。実装の表現力上、使える環境演出は実質「背景色のトーン」のみで、画面に出せないもの(落書き・群衆等)はテキストでも語らない。
...
**本プロジェクトでの使用**: 「タイトル画面 + 1ステージ + ボス1体 + 両手横向きタッチ操作 + PWAオフライン」を MVP の**設計境界**とする。これは初回公開の最小スコープを示す定義であり、現行の実装はこの境界を超えて全6ステージ構成へ拡張されている(物語・ボスの全体像は `docs/story.md` を正とする)。
...
| アクション | 意味 | 使用系統 |
|-----------|------|---------|
| `idle` | 短い静止(行動を読ませる間) | 共通 |
| `move` | 前後に移動してペース(間合いを取り直す。アリーナ端で内向き) | 共通 |
| `shoot` | 弾を発射(phase2 で頻度増) | 共通 |
| `jump` | ジャンプ(前後ドリフト可。重力で着地) | 接地型 |
| `stagger` | 短時間のけぞり(一定ダメージ蓄積時。反撃チャンス) | 共通 |
...
**定義**: 外部音源ファイルを使わず、`src/config/audio.ts` の合成仕様を Web Audio で手続き生成して再生する System レイヤーのコンポーネント(シングルトン。`getSound()` で全シーン横断)。

**説明**: SE(効果音)13種の単発再生と、BGM 5トラック(`title` / `stage` / `stageWarm` / `boss` / `ending`)のループ再生・切替を担う。`GameSettings`(`muted` / `bgmVolume` / `seVolume`)に従って音量・ミュートを反映し、iOS Safari 等の自動再生制約に備え初回ポインタ操作までは無音とする。Entity への参照を持たない出力専用サービスで、`entities/` からの呼び出しを許容する(逆依存・循環は生じない)。音量計算・周波数変換・ノートスケジュール等の純粋ロジックは `src/systems/soundSynth.ts` に分離する。

**関連用語**: [System(システム)](#systemシステム), [localStorage](#localstorage)

**実装**: `src/systems/SoundManager.ts`(純粋ロジックは `src/systems/soundSynth.ts`、データ定義は `src/config/audio.ts`)
