...
**定義**: 画面左半分=追従パッド(左右移動 + 上スワイプでジャンプ)、画面右半分=ショットの仮想ボタン、という分割タッチ操作方式。

**説明**: スマホ横向き(ランドスケープ)・両手持ち専用。左手親指で移動とジャンプ(上スワイプ)、右手親指でショット/チャージを担う。役割を左右で分離したことで、チャージ(右親指の長押し)とジャンプ(左親指の上スワイプ)を同時に行える。

**関連用語**: [方向ゾーン式](#方向ゾーン式), [仮想ボタン](#仮想ボタン)

### 仮想ボタン

**定義**: 画面右半分に配置される、ショットのためのタッチUI要素(ジャンプは左パッドの上スワイプへ移行)。
