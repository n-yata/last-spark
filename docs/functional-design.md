...
> 本書は `docs/product-requirements.md`(PRD)で定義した要件を、Phaser 3 ベースの PWA としてどう実現するかを定義する。スコープは「タイトル画面 + 全6ステージ(stage1〜6、各ステージ固有ボス)」「ステージ選択・周回(New Game+)・難易度(normal/hard)・強化バスターモード」「両手・横向き専用のタッチ操作」「完全オフライン(localStorage)」。
...
| 永続化 | Web Storage API (localStorage) | サーバ不要・端末内完結。完全オフライン方針に合致 |
...
- **外部通信なし**: サーバ通信を行わず、外部送信による情報漏洩リスクを持たない。
