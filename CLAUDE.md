...
1. **作業開始時**: 先に `master` を `git pull` で最新化し、その最新の `master` から `feature` ブランチを切る
   - 命名規則: `feature/<説明>` (例: `feature/stage3-cutscene`)
   - 古い `master` から枝を切ると、後で不要なコンフリクトを生むため、必ず pull を先に行う
...
3. **マージ完了後**: `feature` ブランチを削除する(ローカル・リモート両方)

### 補足

- 軽微な修正でも原則 `feature` ブランチを切る(`master` 直コミットは避ける)。
- 複数セッション・並行作業を同一フォルダで行う場合は、`git worktree` でディレクトリごと分離するのが望ましい(チェックアウトの競合と変更の混在を防ぐ)。
