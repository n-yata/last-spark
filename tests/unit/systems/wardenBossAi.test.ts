...
// stage1 接地ボスとの差別化の要となる固有アクション「missile / containment」が、warden の重みには
// 含まれ、接地/飛行には混入しないこと、phase2 で増えること、連続抑制が効くことを確認する。
...
describe('pickNextWardenBossAction', () => {
  ...
  it('warden の重みには containment が含まれる', () => {
    expect(collect('phase1', 'shoot').has('containment')).toBe(true);
    expect(collect('phase2', 'shoot').has('containment')).toBe(true);
  });
  ...
  it('接地(stage1)ボスには missile が混入しない', () => {
    ...
    expect(all.has('containment')).toBe(false);
  });
  ...
  it('containment は phase2 でより出やすい', () => {
    const countContainment = (phase: 'phase1' | 'phase2'): number => {
      const n = 3000;
      let c = 0;
      for (let i = 0; i < n; i++) {
        if (pickNextWardenBossAction(phase, 'shoot', () => (i + 0.5) / n) === 'containment') c++;
      }
      return c;
    };
    expect(countContainment('phase2')).toBeGreaterThan(countContainment('phase1'));
  });
