---
name: "source-command-add-feature"
description: "新機能を既存パターンに従って、完全に無停止で実装する"
---

# source-command-add-feature

Use this skill when the user asks to run the migrated source command `add-feature`.
...
2. **既存の `requirements.md` を確認する（/plan-feature との連携）:**
  - `Glob('.steering/*/requirements.md')` で、今回の機能名に対応する `requirements.md` を持つディレクトリが既に存在するか確認する。
  - **存在する場合:** そのディレクトリを新規作成せず再利用し、ステアリングディレクトリパスをそのディレクトリに設定する。この `requirements.md` は `/plan-feature` が作成した正式な要求書なので、**削除・上書き・再生成しない**（ステップ4で尊重する）。
...
1. **`requirements.md` が既にあれば最優先で読み込み、尊重する（/plan-feature 連携）:**
  - ステアリングディレクトリに `requirements.md` が存在する場合、`Read` で読み込み、**正式な要求書**として扱う。
  - これは `/plan-feature` でユーザーと合意済みの要求なので、**再生成・上書きしない**。`design.md` / `tasklist.md` 生成の入力としてそのまま使う。
  - `requirements.md` に `## 未決事項` セクションが残っている場合は、設計上の妥当な既定を採用しつつ、判断を振り返り（`retrospective.md`、モード3で作成）に記録する。
2. `AGENTS.md`を読み、プロジェクトの全体像を把握する。
...
1. `Skill('steering')`を**計画モード**で実行し、ステアリングファイルの内容を生成する。
  - **`requirements.md` が `/plan-feature` により既に作成済みの場合:** その内容を**再生成せず尊重**し、`design.md` と `tasklist.md` のみを生成する。
...
1. `Skill('steering')`を**振り返りモード**で実行し、`retrospective.md`（`tasklist.md` とは別ファイル）に申し送り事項を記載する。
  - 実装完了日
  - 計画と実績の差分
  - 学んだこと
  - 次回への改善提案
...
- ステップ8: `retrospective.md` に申し送り事項が記載されている。
