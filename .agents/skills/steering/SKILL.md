...
- 実装完了後の振り返りを **別ファイル（retrospective.md）** へ記録
...
> **軽微な作業でも振り返りは残す**: ステアリングを切るほどでもないバグ修正・小改善でも、
> 残すべき学び・申し送り（再発防止、ハマりどころ、設計判断、次回への注意）があれば、
> モード3で `retrospective.md` **だけ**を作成して `.steering/` に残す（後述「モード3」のケースB）。
...
3. **テンプレートからファイル作成**
   ...
   - `.Codex/skills/steering/templates/requirements.md` → `.steering/[日付]-[機能名]/requirements.md`
   - `.Codex/skills/steering/templates/design.md` → `.steering/[日付]-[機能名]/design.md`
   - `.Codex/skills/steering/templates/tasklist.md` → `.steering/[日付]-[機能名]/tasklist.md`

   ※ `retrospective.md` は計画時には作成しない。実装完了後（モード3）にテンプレートから生成する。
...
2. **振り返りを retrospective.md に記録**
   ```
   テンプレート（.Codex/skills/steering/templates/retrospective.md）を読み込み、
   Writeツールで .steering/[日付]-[機能名]/retrospective.md を作成して記録:
   - 実装完了日
   - 計画と実績の差分
   - 学んだこと
   - 次回への改善提案
   ```
   ※ 振り返りは tasklist.md ではなく retrospective.md に書く（ファイルを分離）
...
- **ケースA: ステアリングを切った作業**（`.steering/[日付]-[機能名]/` に requirements/design/tasklist がある）
  → そのディレクトリ内に `retrospective.md` を作成する。
- **ケースB: ステアリングを切らなかった軽微な作業**（バグ修正・小改善など）
  → 残すべき学び・申し送りが**あれば**、`retrospective.md` 単体を新規ディレクトリに残す。
  → 残すべき内容が**なければ**（typo修正など学びゼロ）作成不要。

> **「残すべき振り返り内容」の基準**: 再発防止につながる気づき、ハマったポイントと回避策、
> 非自明な設計判断とその理由、次回作業への注意点・申し送り。これらが1つでもあれば残す。
...
2. **振り返りテンプレートを読み込む**
   ```
   Read('.Codex/skills/steering/templates/retrospective.md')
   ```
...
5. **ユーザーに報告**
   ```
   「振り返りを retrospective.md に記録しました。内容を確認してください。」
   ```

---

### ケースB: ステアリングを切らなかった軽微な作業の振り返り（retrospective.md 単体）

ステアリングファイル（requirements/design/tasklist）を作らずに済ませたバグ修正・小改善でも、
上記「残すべき振り返り内容」があれば、`retrospective.md` だけを残す。

#### 手順

1. **残すべき内容があるか判断**
   - 学び・申し送りが何もない軽微作業（例: 単純なtypo修正）なら、作成せず終了してよい。

2. **ディレクトリ名を決める**
   - `.steering/[YYYYMMDD]-[作業名]/` を新規作成（命名規則は通常作業と同じ。例: `20260614-fix-laser-sound`）。
...
5. **ユーザーに報告**
   ```
   「軽微な作業の振り返りを retrospective.md として残しました。内容を確認してください。」
   ```
